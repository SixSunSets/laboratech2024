...
output_path = f"planilla_Electrónica_meses_{str(fechas['fecha_inicio'])}-{str(fechas['fecha_fin'])}_obs.docx"
...
('Observaciones', {'bold': True, 'font_size': 12}),
('Alertas', {'bold': True, 'font_size': 12}),
...
obs, alert = resp[f"{fechas['fecha_fin']}"]['observaciones'], resp[f"{fechas['fecha_fin']}"]['alertas']


titulo_alertas = doc.add_paragraph("Alertas")
for al in alert:
    alerta_paragraph = doc.add_paragraph(al)

doc.add_paragraph("")

titulo_observaciones = doc.add_paragraph("Observaciones")
for ob in obs:
    obs_paragraph = doc.add_paragraph(ob)


for texto, propiedades in formatos:
        apply_format_to_text(doc, texto, **propiedades)

doc.save(f"planilla_Electrónica_meses_{str(fechas['fecha_inicio'])}-{str(fechas['fecha_fin'])}_obs.docx")
nombre_docx = f"planilla_Electrónica_meses_{str(fechas['fecha_inicio'])}-{str(fechas['fecha_fin'])}_obs.docx"
convert(nombre_docx)

# Nombre del archivo generado dinámicamente
file_name = f"planilla_Electrónica_meses_{str(fechas['fecha_inicio'])}-{str(fechas['fecha_fin'])}_obs.docx"
