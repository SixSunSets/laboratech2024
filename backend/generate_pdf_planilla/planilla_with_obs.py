...
def process_text(row):
    ...
    doc.save(f"planilla_Electrónica_{str(fechas['fecha'])}_obs.docx")
...
doc.add_paragraph("Multas Laborales")
...
obs, alert = resp[f"{fechas['fecha']}"]['observaciones'], resp[f"{fechas['fecha']}"]['alertas']
...
doc.save(f"planilla_Electrónica_{str(fechas['fecha'])}_obs.docx")
nombre_docx = f"planilla_Electrónica_{str(fechas['fecha'])}_obs.docx"
convert(nombre_docx)

# Nombre del archivo generado dinámicamente
file_name = f"planilla_Electrónica_{str(fechas['fecha'])}_obs.docx"
